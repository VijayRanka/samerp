			          ...
			          <div class="control-group" >
			            ...
			            <div class="control-group">
			              ...
			              <div class="controls">
			                ...
			                <option value="PIPE PURCHASE">Pipe Purchase</option>
			            ...
			            <div class="control-group" style="">
				             	...
				             	<div class="control-group" id="selectIndividual" style="display: none">
					               ...
					               <div class="controls">
					                 <input type="text" list="getList" id="individualName" autocomplete="off"  placeholder="Individual Name" />
	...
	if(value!=='PAYMENTSTATEMENT'){
		...
		xhttp.onreadystatechange = function() {
			...
			if(value=='EXPENSES')
				...
				xhttp.open("POST", "/SAMERP/ContractorPayment?findList=1", true);
			else if(value=='VEHICLE REPAIR')
				xhttp.open("POST", "/SAMERP/AddVehicles?ReportByVehicleName=1", true);
			else if(value=='PIPE PURCHASE')
				xhttp.open("POST", "/SAMERP/ProductPurchase?ReportBysupplierName=1", true);
		xhttp.send();
...
function getBillReportData() {
	...
	// for Production details vijay
	else if(document.getElementById("reportType").value=='PRODUCTDETAILS')
	{
		 if(document.getElementById("mode1").checked==true)
			{
	...
	// Mukesh for Vehicle Repair report
	else if(document.getElementById("reportType").value=='VEHICLE REPAIR')
	{
		if(document.getElementById("mode1").checked==true)
		{
			getVehicleData();
	...
	// Mukesh for pipe purchase report
	else if(document.getElementById("reportType").value=='PIPE PURCHASE')
	{
		if(document.getElementById("mode1").checked==true)
		{
			getPipePurchaseData();
...
// 	   newWin.close();
     	...
     	var lastDate=document.getElementById("toDate").value;
  		...
  		xhttp.onreadystatechange = function() {
  			if (this.readyState == 4 && this.status == 200) {
  			    ...
  			    "<th>Product Name</th>"+
  				"<th>Product Qty</th>"+
  				"<th>Prod/Unit Rate</th>"+
  				"<th>Querying Qty</th>"+
  				"<th>Query/Unit Rate</th>"+
  				"</thead><tbody>";
  				var count=1;
  				
  				 var main=demoStr.split("#");
  				for(var i=1;i<main.length;i++)
  					{
  						var dateAndCont=main[i].split(".")[0];
  						var date=dateAndCont.split("/")[0];
  						var contName=dateAndCont.split("/")[1];
  						
  						var wholeData=main[i].split(".")[1];
  						var qtyPro=wholeData.split("/");
  						
  						a+= "<tr>"+
		  					"<td style='text-align: center' rowspan='"+(qtyPro.length-1)+"'>"+count+"</td>"+
		  					"<td style='text-align: center' rowspan='"+(qtyPro.length-1)+"'>"+date+"</td>"+
		  					"<td style='text-align: center' rowspan='"+(qtyPro.length-1)+"'>"+contName+"</td>";
		  					
		  					for(var j=0;j<qtyPro.length-1;j++)
  							{ 
		  						var l=0;
		  						var totalAmount=0;
		  						var lastData=qtyPro[j].split(",");
		  						
		  						
		  						if(j<1){
		  							a+="<td style='text-align: center'>"+lastData[0]+"</td>"+
				  					"<td style='text-align: center'>"+lastData[1]+"</td>"+
				  					"<td style='text-align: center'>"+lastData[2]+"</td>";
				  					if(lastData[3]=="0")
				  					a+="<td style='text-align: center'>-</td>";
				  					else
				  						a+="<td style='text-align: center'>"+lastData[3]+"</td>";
				  					if(lastData[4]=="0")
					  					a+="<td style='text-align: center'>-</td>";
					  					else
					  						a+="<td style='text-align: center'>"+lastData[4]+"</td>";
		  							
		  						}
		  						else{
		  							a+="<tr><td style='text-align: center'>"+lastData[0]+"</td>"+
				  					"<td style='text-align: center'>"+lastData[1]+"</td>"+
				  					"<td style='text-align: center'>"+lastData[2]+"</td>";
				  					if(lastData[3]=="0")
					  					a+="<td style='text-align: center'>-</td>";
  				...
  				document.getElementById("reportDetails").innerHTML="<span style='color:#f73838'>PRODUCT DETAILS"+
  				"</span> REPORT FROM: <span style='color:#f73838'>"+reportFirstDate +"</span> TO: <span style='color:#f73838'>"+
  				reportLastDate+"</span>";
  			}
  				
  			};
  		xhttp.open("POST", "/SAMERP/AddDailyStock?getReportData=1&fromDate="+firstDate+"&toDate="+lastDate, true);
  		xhttp.send();
      	 
     	 
      }
     
     function getProductionDetailsByName()
     {
      	var firstDate=document.getElementById("fromDate").value;
      	var lastDate=document.getElementById("toDate").value;
      	var contAlias=document.getElementById("individualName").value;
   		var xhttp;
   		xhttp = new XMLHttpRequest();
   		xhttp.onreadystatechange = function() {
   			if (this.readyState == 4 && this.status == 200) {
   		...
   		xhttp.open("POST", "/SAMERP/AddDailyStock?getReportData=1&fromDate="+firstDate+"&toDate="+lastDate+"&wholeData="+contAlias, true);
   		xhttp.send();
  ...
  function getBankDataByName()
	  	...
	  	xhttp.onreadystatechange = function() {
				if (this.readyState == 4 && this.status == 200) {
					...
					if(demoStr=="")
						...
						var count=1;
	  					for(var i=0;i<demoStr.length-2;i=i+11)
  						{
	  						 a+= "<tr>"+
	  						"<td style='text-align: center'>"+count+"</td>"+
	  						"<td style='text-align: center'>"+demoStr[i]+"</td>"+
	  		...
	  		xhttp.open("POST", "/SAMERP/AddVehicles?getDateData=1&fromDate="+firstDate+"&toDate="+lastDate, true);
			xhttp.send();
	}
			
	
	function getDataByVehicle()
    {
    	var firstDate=document.getElementById("fromDate").value;
 		var lastDate=document.getElementById("toDate").value;
	   	var name=document.getElementById("individualName").value;
		...
		xhttp.open("POST", "/SAMERP/AddVehicles?getDateData=1&individualName="+name+"&fromDate="+firstDate+"&toDate="+lastDate, true);
		xhttp.send();
     }
     
	
	function getPipePurchaseData()
    {
	...
	function getPipePurchaseDataBySupplier()
    {
		...
		xhttp.open("POST", "/SAMERP/ProductPurchase?getDateData=1&individualName="+name+"&fromDate="+firstDate+"&toDate="+lastDate, true);
		xhttp.send();
    }
	
						
	//*************  MUKESH VEHICLE REPORT END *******************
